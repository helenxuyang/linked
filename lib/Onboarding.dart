...
class _OnboardingPageState extends State<OnboardingPage> {
  ...
  int gradYear;
  List<String> statuses = ['Living on-campus', 'Living off-campus in Ithaca', 'Living outside of Ithaca'];
  String f20Status;
  List<String> tags;
  List<String> classes;
  ...
  Widget _buildInfoPage(BuildContext context) {
    return ListView(
        ...
        children: [
          ...
          smallSpacer,
          Text('These will be shown on your profile for others to get to know you!', style: Theme.of(context).textTheme.headline2),
          bigSpacer,
          Text('Intended major'),
          smallSpacer,
          TextField(
          ...
          DropdownButton(
            value: f20Status,
            items: statuses.map((str) => DropdownMenuItem(
              child: Text(str),
              value: str,
            )).toList(),
            onChanged: (selection) {
  ...
  Widget _buildEventPage(BuildContext context) {
    ...
    List<String> allTags = ['video games', 'board games', 'music', 'tv show', 'movies', 'study group', 'making friends'];

    return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('What kinds of events are you interested in?', style: Theme.of(context).textTheme.headline1),
          smallSpacer,
          Text('We\'ll recommend events for you based on your preferences!', style: Theme.of(context).textTheme.headline2),
          bigSpacer,
          Text('Type'),
          smallSpacer,
          CheckboxListTile(
            ...
            onChanged: (value) {
              setState(() {
                if (value) tags.add(inPersonTag);
                else tags.remove(inPersonTag);
              });
            },
          ),
          CheckboxListTile(
            ...
            onChanged: (value) {
              setState(() {
                if (value) tags.add(virtualTag);
                else tags.remove(virtualTag);
              });
            },
          ),
          bigSpacer,
  ...
  Widget _buildClassPage(BuildContext context) {
    return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('What classes are you taking?', style: Theme.of(context).textTheme.headline1),
          smallSpacer,
          Text('We\'ll show you study groups for the classes you\'re taking!', style: Theme.of(context).textTheme.headline2),
          bigSpacer,
          Text('Classes'),
          ListView.builder(
              shrinkWrap: true,
        ...
        builder: (context) {
          return FlatButton(
            ...
            onPressed: () {
              ...
              switch(currentIndex) {
                case 0:
                  if (firstNameCtrl.text == null || firstNameCtrl.text.isEmpty || lastNameCtrl.text == null || lastNameCtrl.text.isEmpty) {
                    valid = false;
                    Scaffold.of(context).showSnackBar(SnackBar(content: Text('Please enter your first and last name!')));
                  }
                  break;
                case 1:
                  if (majorCtrl.text == null || majorCtrl.text.isEmpty) {
                    valid = false;
                    Scaffold.of(context).showSnackBar(SnackBar(content: Text('Please enter your major, feel free to put \'Undecided\' if you\'re unsure!')));
                  }
                  break;
                default:
                  break;
