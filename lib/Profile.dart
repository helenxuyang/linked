...
class ProfilePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    TextStyle subtitleStyle = TextStyle(fontSize: 20, fontWeight: FontWeight.bold);
    return Padding(
            ...
            builder: (context, snapshot) {
              ...
              return Expanded(
                child: Scrollbar(
                  child: ListView(
                      children: [
                        Text(doc.get('firstName') + ' ' + doc.get('lastName'), style: Theme.of(context).textTheme.headline1),
                        Text(doc.get('major') + ' ' + doc.get('classYear').toString(), style: TextStyle(fontSize: 18)),
                        Text(doc.get('status'), style: TextStyle(fontSize: 18, color: Color.fromRGBO(0xeb, 0x8a, 0x90, 1.0))),
                        SizedBox(height: 16),
                        ...
                        Wrap(
                          children: List<String>.from(doc.get('interestedTags')).map((str) => InputChip(label: Text(str))).toList(),
                          spacing: 4,
                          runSpacing: -8,
                        ),
                        SizedBox(height: 16),
                        ...
                        Wrap(
                          children: List<String>.from(doc.get('classes')).map((str) => InputChip(label: Text(str))).toList(),
                          spacing: 4,
                          runSpacing: -8,
                        ),
                        SizedBox(height: 16),
