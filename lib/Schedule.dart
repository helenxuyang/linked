        ...
        StreamBuilder(
            ...
            builder: (context, snapshot) {
              ...
              if (eventDocs.isEmpty) {
                return Text(
                    'No events yet 😥 Sign up for one to join the fun!');
              }
              Map<DateTime, List<DocumentSnapshot>> daysAndEvents = {};
              for (DocumentSnapshot d in eventDocs) {
                DateTime dateTime = d.get('startTime').toDate();
                DateTime justDay =
              ...
              List<DateTime> daysList = daysAndEvents.keys.toList()
                ..sort((d1, d2) => d1.compareTo(d2));
              List<List<DocumentSnapshot>> eventDocsList = [
                for (DateTime d in daysList) daysAndEvents[d]
              ];
              return Expanded(
                child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return Column(
                        ...
                        children: [
                          Text(DateFormat('EEEE').format(daysList[index]),
                              style: TextStyle(fontSize: 14)),
                          Text(DateFormat('d').format(daysList[index]),
                              style: TextStyle(fontSize: 18)),
                          SizedBox(height: 6),
                          ListView(
                              physics: NeverScrollableScrollPhysics(),
                              shrinkWrap: true,
                              children: eventDocsList[index]
                                  .map((eventDoc) =>
                                      EventRow(Event.fromDoc(eventDoc)))
                                  .toList()),
                          SizedBox(height: 16)
                        ],
                      );
